...
def _install_linux_systemd(plugin_root: str) -> None:
    ...
    shutil.copy(Path(plugin_root) / "compile-wiki-auto.sh", script)
    script.chmod(0o755)
    ...
    print("Wiki auto-compile: systemd user timer enabled (daily 07:00).")
...
def _install_macos(plugin_root: str) -> None:
    ...
    print(f"Wiki auto-compile: launchd agent loaded ({plist_dst}).")
...
def _install_windows(plugin_root: str) -> None:
    ...
    shutil.copy(Path(plugin_root) / "compile-wiki-auto.ps1", script)
